        ...
        if [[ -f /root/ca/cert.crt && -f /root/ca/private.key ]] && [[ -s /root/ca/cert.crt && -s /root/ca/private.key ]] && [[ -f /root/ca/ca.log ]]; then
            ...
            if [[ $domainIP == $ip ]]; then
                ...
                if [[ -f /root/ca/cert.crt && -f /root/ca/private.key ]] && [[ -s /root/ca/cert.crt && -s /root/ca/private.key ]]; then
                    ...
                    green "证书申请成功! 脚本申请到的证书 (cert.crt) 和私钥 (private.key) 文件已保存到 /root/ca 文件夹下"
...
insthysteria(){
    ...
    wget -N https://raw.githubusercontent.com/Jason6111/ExpressSetup/main/hysteria/install_server.sh
...
EOF
    ...
    url="hysteria2://$auth_pwd@$last_ip:$last_port/?insecure=1&sni=$hy_domain#Hysteria2$hy_domain"
